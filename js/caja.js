		...
		$("#guardar_cobro").submit(function(event) {
			$('#guardar_datosc').attr("disabled", true);
			var parametros = $(this).serialize();
			$.ajax({
				type: "POST",
				url: "../ajax/nuevo_clientex.php",
				data: parametros,
				beforeSend: function(objeto) {
					$("#resultados_ajaxc").html('<img src="../../img/ajax-loader.gif"> Cargando...');
				},
				success: function(datos) {
					$("#resultados_ajaxc").html(datos);
					$('#guardar_datosc').attr("disabled", false);
            //resetea el formulario
            $("#guardar_cobro")[0].reset();
            //desaparecer la alerta
            $(".alert-success").delay(400).show(10, function() {
            	$(this).delay(2000).hide(10, function() {
            		$(this).remove();
